// Command gen-skills writes the 45 AgentFlow skill files (9 skills × 5 agent
...
func main() {
	...
	for _, def := range allSkills {
		...
		if err := writeFile(filepath.Join(outDir, "opencode", def.Name, "SKILL.md"), skillgen.Render(def, skillgen.FlavorOpenCode)); err != nil {
			fail(err)
		}
	}

	fmt.Printf("Generated %d skill files (%d skills x 5 flavors) in %s\n", len(allSkills)*5, len(allSkills), outDir)
}
