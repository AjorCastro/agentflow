...
- Once Phase 3 (implementation) is approved — `STATUS.md` says phase `done`, next action mentions merge — that approval already covers the PR and the merge. Open the PR if repo policy requires one, merge it, then validate before closing: check the repository root for `AGENTS.md` or `CONTRIBUTING.md` (or docs they point to) for a mandatory local and/or remote validation procedure — installed git hooks, required test/build commands, a CI workflow that must go green on the branch you merged into, or a promotion gate to further environments. Follow whatever that procedure requires and wait for it to pass. If the repository documents no such procedure, use your judgment based on whatever test/CI setup already exists. Never bypass a documented hook or gate (e.g. `--no-verify`) to force things through. Only once that gate is green — or there is none — run `agentflow close`. Do not create a decision request or otherwise ask the Web Reviewer to re-approve the PR, the merged result, or a green gate; only escalate via a `decisions/` request if something unexpected happens (conflicts, a gate that stays red after a fix attempt, a procedure you can't complete).
...
```
handoffs/handoff-<YYYY-MM-DD>-kimi-cli.md
```
With this content:
```markdown
# Handoff — <phase> — <date>
